 ...
 */
@Controller
@RequestMapping("/customFileTypeAttributes")
public class CustomFileAttributesController {
  ...
  public ModelAndView displayCustomFileTypeAttributesForm(Model model, RedirectAttributes redirectAttrs,
    ...
    model.addAttribute("parsedData", wizardManager.parseDataFileByLine(rosettaCookie.getValue()));
  ...
  /**
   * Accepts a POST request from custom file type attribute collection step of the wizard. Processes the submitted data
   * and persists it to the database. Redirects user to next step or previous step depending on submitted form button
   * (Next or Previous).
   * <p>
  ...
  public ModelAndView processCustomFileTypeAttributes(WizardData wizardData, @RequestParam("submit") String submit,
